...
def get_gares_from_api(nom_commune):
    """
    Récupère les gares d'une commune via l'API OpenData de la SNCF.
    """
    url = "https://ressources.data.sncf.com/api/records/1.0/search/"
    params = {
        "dataset": "liste-des-gares",
        "rows": 100,  # Nombre max de gares à récupérer
        "q": nom_commune  # Filtrer par nom de la ville
    }
    ...
    if response.status_code == 200:
        ...
        for record in data.get("records", []):
            ...
            if "geo_point_2d" in fields:
                gares.append({
                    "nom": fields.get("libelle", "Gare sans nom"),
                    "latitude": fields["geo_point_2d"][0],
                    "longitude": fields["geo_point_2d"][1],
                    "uic": fields.get("code_uic", "N/A"),  # Identifiant unique de la gare
                    "voyageurs": "Oui" if fields.get("voyageurs") == "O" else "Non",
                    "fret": "Oui" if fields.get("fret") == "O" else "Non",
                    "departement": fields.get("departemen", "Inconnu"),
                    "code_ligne": fields.get("code_ligne", "N/A")  # Code de la ligne SNCF
                })
